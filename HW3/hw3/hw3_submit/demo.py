...
def train(cfg:DQNConfig,env,agent):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        # for i_step in range(cfg.train_steps):
            ...
            reward = 0
            # Successful landing if both legs touch the ground and the tilt is small
            if next_observation[6] and next_observation[7] and abs(next_observation[0]) < 0.1:
                reward += 10000  # Large reward for a smooth landing
            elif next_observation[6] and next_observation[7]:
                reward += 200
            # Penalties for improper landings
            elif truncated:
                reward -= 500  # Penalize for crashes
            reward -= 5 * (abs(next_observation[4]) + abs(next_observation[5])) + 30 * abs(next_observation[0])
            # Encourage steady descent (reward for low vertical speed)
            if abs(next_observation[3]) > 0.2:  # next_observation[3] is the vertical speed
                reward -= 2
            # Penalize for lateral movement (next_observation[2] is horizontal speed)
            if abs(next_observation[2]) > 0.1:
                reward -= 1  # Discourage high horizontal speed
            if next_observation[1] < 0.9 and next_observation[1] > 0.2 and abs(next_observation[3]) > 0.04 and action == 2:
                reward += 300
