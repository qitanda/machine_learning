...
def img2obs(image_array):
    ...
    #print(agent_pos, goal_pos, adv_pos)#[668 275] [199 599] [599 280]
    obstacle_pos = []
    distance = []
    ...
    #print(obstacle_pos)#[array([259, 259]), array([539, 259]), array([399, 539])]
...
#     cv2.imshow("con",np.concatenate((goal_pos-agent_pos,agent_pos,adv_pos-agent_pos,obstacle_pos[sorted_indexes[0]]-agent_pos,obstacle_pos[sorted_indexes[1]]-agent_pos,obstacle_pos[sorted_indexes[2]]-agent_pos))/256
#   )
#     cv2.waitKey(1)
...
if __name__ == "__main__":
    ...
    if cfg.train:
        ...
        if cfg.load_model:
            print(">>>>>>>>>>load model<<<<<<<<<<<<<<<")
            agent.load(path=cfg.model_path,i_ep=72)
        ...
        ss_epoch = 0
        i_step_all = 0
        for i in range(166,170,1):
        # for i in range(9992,10000,8):
            print(i)
            epoch +=1
