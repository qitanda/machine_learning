...
# def train(cfg:PPOConfig, env, agent):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        # 初始化上一步的距离
        previous_distance_to_goal = float('inf')
        previous_distance_to_adv = float('inf')
        ...
        while True:
            ...
            # 计算与目标和追击者的距离
            goal_pos = next_obs_n[0]['goal_position']  # 这里假设你的环境返回目标位置
            agent_pos = next_obs_n[0]['agent_position']  # 这里假设你的环境返回智能体位置
            adv_pos = next_obs_n[0]['adv_position']  # 这里假设你的环境返回追击者位置

            current_distance_to_goal = np.linalg.norm(agent_pos - goal_pos)
            current_distance_to_adv = np.linalg.norm(agent_pos - adv_pos)

            # 奖励计算
            reward = reward_n[1]  # 从环境中获得原始奖励
            ...
            # 处理与追击者的距离
            if current_distance_to_adv < previous_distance_to_adv:
                reward -= 1  # 被追击者接近，给予惩罚
            
            # 更新上一步的距离
            previous_distance_to_goal = current_distance_to_goal
            previous_distance_to_adv = current_distance_to_adv
